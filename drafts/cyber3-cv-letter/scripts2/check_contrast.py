...
def parse_named_colors(source: str) -> dict[str, str]:
    ...
    `note-tint = muted`, which resolve to an already-checked token)."""
    colors = {}
...
def parse_accent_presets(source: str) -> dict[str, list[str]]:
    """The accent-presets dict maps each preset name to a list of hex
    colours (single-colour presets and multi-colour rotations alike)."""

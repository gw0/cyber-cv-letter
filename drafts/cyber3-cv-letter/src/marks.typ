...
// Sized off height alone (entry logo cells are always 3em × 2em landscape).
#let draw-placeholder-logo(color) = artifact(
  box(width: 3em, height: 2em, {
    place(center + horizon, circle(radius: 2em * 0.3, fill: color))
    place(center + horizon, circle(radius: 2em * 0.12, fill: bg))
  }),
  kind: "other",
)
